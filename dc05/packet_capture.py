...
class TCPHeader(BaseFrame):
    def __init__(self, buf):
        ...
        self.URG = (int(packet[5].encode('hex'), 16) >> 5) & 0x1
        self.ACK = (int(packet[5].encode('hex'), 16) >> 4) & 0x1
        self.PSH = (int(packet[5].encode('hex'), 16) >> 3) & 0x1
        self.RST = (int(packet[5].encode('hex'), 16) >> 2) & 0x1
        self.SYN = (int(packet[5].encode('hex'), 16) >> 1) & 0x1
        self.FIN = (int(packet[5].encode('hex'), 16)) & 0x1
